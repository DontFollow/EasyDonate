...
class Updates extends Controller
{
     ...
     * Index controller
     */
    public function index($update = false, $updates = false)
    {
        try {
            $update = $this->client->post("http://easydonate.ru/api/v2/updates/check?{$this->credentials}")->getBody()->getContents();
            $update = json_decode($update);
            $updates = $this->client->post("http://easydonate.ru/api/v2/updates/get?{$this->credentials}")->getBody()->getContents();
    ...
    public function onExecuteStep()
    {
        ...
        switch ($stepCode) {
            case 'updateCheck':
                $manager->updateCheck($this->credentials);
                break;
            ...
            case 'prepareUpdateFile':
                $manager->prepareUpdateFile($this->credentials);
                break;

            case 'downloadUpdateFile':
                $manager->downloadUpdateFile($this->credentials);
                break;

            case 'prepareForUpdate':
                $manager->prepareForUpdate();
                break;

            case 'extractUpdateFile':
                $manager->extractUpdateFile();
                break;
     ...
     */
    public function onCheckForUpdates($update = false)
    {
        try {
            $update = Http::get("http://easydonate.ru/api/v2/updates/check?{$this->credentials}");
        } catch (Exception $ex) {
            $this->handleError($ex);
        }
        ...
        $this->vars['hasUpdates'] = $update->status;
        $this->vars['update'] = $update->status ? $update->newVersion : false;
    ...
    protected function buildUpdateSteps($core, $plugins, $themes, $isInstallationRequest)
    {
        return [
            ...
            [
                'code' => 'prepareUpdateFile',
                'label' => 'Подготовка установочного файла'
            ],
            [
                'code' => 'downloadUpdateFile',
                'label' => 'Скачивание установочного файла'
            ],
            [
