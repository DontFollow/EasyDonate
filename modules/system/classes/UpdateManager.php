...
class UpdateManager
{
    ...
    public function updateCheck($credentials, $url = 'http://easydonate.ru/api/v2/updates/check')
    {
        try {
            $response = Http::get(implode('?', [$url, $credentials]));
    ...
    {
        $backupPath = storage_path('app/backup.zip');
        ...
        if ($zip->open($backupPath, ZipArchive::CREATE) !== true) {
            throw new ApplicationException('Не удалось создать бэкап файл.');
        }

        $files = Storage::disk('base')->allFiles();
        if (!$files) {
            throw new ApplicationException('Не удалось отсканировать файловую структуру сайта.');
        }
    ...
    public function prepareUpdateFile($credentials, $url = 'http://easydonate.ru/api/v2/installation/prepare')
    {
        try {
            $response = Http::get(implode('?', [$url, $credentials]));
            $response = json_decode($response);
        ...
        if (!isset($response->hash)) {
            throw new ApplicationException('Не удалось установить хэш-сумму файла.');
        }

        Cache::put('hash', $response->hash, 3600);
    }

    public function downloadUpdateFile($credentials, $url = 'http://easydonate.ru/api/v2/installation/download')
    {
        if (!Cache::has('hash')) {
            throw new ApplicationException('Не удалось установить хэш-сумму файла.');
        }

        $downloadFile = temp_path('download.zip');
        if (file_exists($downloadFile)) {
            unlink($downloadFile);
        }

        try {
            $download = Http::get(implode('?', [$url, $credentials]));
            file_put_contents($downloadFile, $download);
        } catch (Exception $e) {
            throw new ApplicationException($e->getMessage());
        }

        chmod($downloadFile, 0777);

        $hash = Cache::get('hash');
        if (hash_file('sha256', $downloadFile) != $hash) {
            throw new ApplicationException('Хэш-суммы не совпадают. Возможно, установленный архив пуст или скачан не полностью.');
        }
    }

    public function prepareForUpdate()
    {
        $downloadedFile = temp_path('download.zip');
        if (!file_exists($downloadedFile)) {
            throw new ApplicationException('Установочный файл не найден.');
        }

        $zip = new ZipArchive;
        if ($zip->open($downloadedFile) !== true) {
            throw new ApplicationException('Установочный файл поврежден.');
        }

        $zip->deleteName('config/credentials.php');
        $zip->deleteName('config/database.php');
        $zip->deleteName('config/app.php');
        $zip->deleteName('config/cms.php');
        $zip->deleteName('.htaccess');
        $zip->close();
    }

    public function extractUpdateFile()
    {
        $extractFile = temp_path('download.zip');
        ...
        if ($zip->open($extractFile) !== true) {
            throw new ApplicationException('Не удалось открыть установочный файл. Возможно, он поврежден.');
        }
